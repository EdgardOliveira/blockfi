...
import { AddCircleOutline, Beenhere, Cancel, DeleteForever, Edit } from '@material-ui/icons';
...
export default function ListarGrupo({ grupos }: any) {
  ...
  async function excluir(id: number) {
    try {
      const resposta = await excluirDados('/api/grupos', id);

      if (resposta.sucesso === true) {
        // handleResponse('success', `Rede: ${resposta.grupo[0].nome} excluído(a) com sucesso!`);
  ...
  return (
    <LayoutWithMenu>
      <div className={classes.toolbar}>
        ...
        <div>
          <Link href='/grupos/cadastrar' passHref>
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  {row.status == 'Ativo' ?
                    <IconButton aria-label='ativo'><Beenhere /></IconButton> :
                    <IconButton aria-label='inativo'><Cancel /></IconButton>
                  }
                </TableCell>
                <TableCell>
                  <IconButton
                  ...
                  <Link href={`/grupos/editar/${row.id}`} passHref>
...
ListarGrupo.getInitialProps = async (context: NextPageContext) => {
  let url;
  if (!process.env.BASE_URL)
    url = '/api/grupos';
  else
    url = `${process.env.BASE_URL}/api/grupos`;

  const res = await getCookies(url, context);
  const json: Grupo[] = res.grupos;
